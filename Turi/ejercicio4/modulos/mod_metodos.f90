...
subroutine falsa_posicion(f,a,b,tol_x,tol_y,raiz,max_ite,Nite,unidad)
...
!********************************************************
!METODO DE NEWTON MODIFICADO (NEWTON-RAPHSON)
!********************************************************

subroutine newton_m(f,df,d2f,a,tol_x,tol_y,raiz,max_ite,Nite,unidad)

!*************************************************************************
! f: funcion a encontrar la raiz
! df: derivada de la funcion
! d2f: derivada 2da de la funcion
! a : punto por el que empieza a funcionar
! tol_x y tol_y: el error maximo permitido en x e y
! raiz: resultado final del programa
! max_ite: numero maximo de iteraciones permitidas
...
integer(kind=id),intent(out)  :: Nite
integer(kind=is),intent(in)   :: unidad
real(kind=rd)   :: f, df, d2f

!defino variables auxiliares
real(kind=rd) :: fa, dfa, d2fa, p, fp, errorx, errory
real(kind=rd) :: ai
...
do
 p = ai - (fa * dfa) / ((dfa) ** 2.0_rd - fa * d2fa)
 fp = f(p)
