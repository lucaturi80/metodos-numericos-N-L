...
subroutine biseccion(f,aext,bext,tol_x,tol_y,p,fp,ite,max_ite,error_r_x,error_a_y,exito)
    ...
    if(fa*fb > 0) then
        ...
        open(unit=1,file="datos/datos_biseccion.dat")
        do ite=1,MAX_ITE
            p = a + (b-a)/2.
            fp= f(p)
            if(fp == 0.0_wpr) exit
...
subroutine secante(f,aext,bext,tol_x,tol_y,p,fp,ite,max_ite,error_r_x,error_a_y,exito)
    ...
    if(fa*fb > 0) then
    	...
    	open (unit=2, FILE = "datos/datos_secante.dat")
        do ite=1,MAX_ITE
            p1 = a - (fa * (b - a)) / (fb - fa)
            fp= f(p)
            ...
            write(2,66) ite,p,fp,error_r_x,error_a_y, a, b
...
subroutine regula_falsi(f,aext,bext,tol_x,tol_y,p,fp,ite,max_ite,error_r_x,error_a_y,exito)
    ...
    if(fa*fb > 0) then
    	...
    	open(UNIT = 3, FILE = "datos/regula_falsi.dat")
        do ite=1,MAX_ITE
            p = p1
            p1 = a - (fa * (b - a)) / (fb - fa)
            fp= f(p)
            ...
            write(*,66) ite,p,fp,error_r_x,error_a_y, a, b
            write(3,66) ite,p,fp,error_r_x,error_a_y, a, b
66          FORMAT(I4, 6(2x,E18.12))
...
fp = f(p)
dfp = df(p)
open(UNIT = 4, FILE = "datos/newton-raphson.dat")
DO ite=1,max_ite
...
dfp = df(p)
ddfp = ddf(p)
OPEN(unit = 5, FILE = "datos/newton_modificado.dat")
DO ite=1,max_ite
    ...
    write(5,66) ite, p1, fp, error_a_y, error_r_x
66  FORMAT(I4, 4(2x,E18.12))
    if ( error_r_x < tol_x  .and. error_a_y < tol_y) exit
